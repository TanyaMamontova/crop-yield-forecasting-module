...
crop_data = {
    ...
    "Картофель": {
        ...
        2023: 208.77, 2024: 270.0, 2025: 237.70, 2026: 274.58
    },
    "Кукуруза на зерно": {
    ...
    "Подсолнечник": {
        ...
        2023: 22.9, 2024: 14.60, 2025: 20.23, 2026: 21.01
    },
    "Просо": {
    ...
    "Сахарная свекла": {
        ...
        2023: 536.65, 2024: 409.80, 2025: 508.91, 2026: 460.70
    },
    "Соя": {
...
def analytics_data():
    ...
    end_year = min(data.get('end_year', 2023), 2024)  # ограничиваем максимумом 2024
...
@app.route('/api/crop/<crop>')
@login_required
def crop_data_api(crop):
    data = crop_data.get(crop, {})
    filtered = {year: val for year, val in data.items() if year <= 2024}
...
def forecast():
    filtered_crop_data = {
        k: v for k, v in crop_data.items()
        if k in ["Картофель", "Сахарная свекла", "Подсолнечник"]
    }

    if request.method == 'POST':
        crop = request.form['crop']
        crop_yields = filtered_crop_data.get(crop, {})

        # Преобразуем данные в списки, сортируем по годам
        years = sorted(crop_yields.keys())
        values = [crop_yields[year] for year in years]
        ...
        return render_template(
            ...
            years=years,
            values=values,
            prediction_2025=crop_yields.get(2025),
            prediction_2026=crop_yields.get(2026),
            crop_data=filtered_crop_data
        )

    return render_template('forecast.html', crop_data=filtered_crop_data)
...
def compare():
    crops = list(crop_data.keys())
    # Оставляем только годы до и включая 2024
    years = sorted({year for data in crop_data.values() for year in data.keys() if year <= 2024})
...
def compare_data():
    data = request.get_json()
    selected_crops = data.get('crops', [])
    selected_years = [int(y) for y in data.get('years', []) if int(y) <= 2024]  # ← фильтрация здесь
